...
class Admin extends CI_Controller {
	...
	function jenis_surat(){
		...
		$a['page']	= "jenis_surat";
		 $a['tb_jenis_surat'] = $this->surat_masuk->getAll();
...
*/
   ...
   //     if (!isset($id)) redirect('admin/disposisi');
        ...
        $tb_jenis_surat = $this->surat_masuk;
      $validation = $this->form_validation;
      $validation->set_rules($tb_jenis_surat->rules());
       if ($validation->run()) {
            $tb_jenis_surat->update();
            $this->session->set_flashdata('success', 'Berhasil disimpan');
      }
       $data["tb_jenis_surat"] = $tb_jenis_surat->getById($id);
        //if (!$data["gambar"]) show_404();
